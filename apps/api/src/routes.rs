...
use crate::{
    app_handlers::{
        submit_app_auto_archive, submit_app_create_encounter, submit_app_create_media_ticket,
        submit_app_create_organization, submit_app_create_patient, submit_app_create_project,
        submit_app_create_provider, submit_app_create_site, submit_app_delete_site,
        submit_app_project_toggle_dormancy, submit_app_send_invite,
        submit_app_site_toggle_dormancy, submit_attach_site_to_project,
        submit_detach_site_from_project, submit_reset_site_checklist,
    },
    ...
    db::Db,
    error::ApiError,
    models::{DataUseAgreement, DataUseAgreementSignature, Organization, OutboundEmail},
    query_handlers::{
        close_study_data_query, create_study_data_query, list_study_data_queries,
        respond_study_data_query,
    },
    study_handlers::{
        generate_sample_answers_json, options_json_to_lines, render_crf_field_for_data_entry,
        render_crf_field_type_options, submit_add_study_crf_field,
        submit_bulk_delete_corrupted_study_crf_fields, submit_bulk_delete_study_crf_fields,
        submit_close_study_data_query, submit_create_study_crf_submission,
        submit_create_study_crf_template, submit_create_study_data_query,
        submit_create_study_from_ui, submit_create_study_visit_template,
        submit_import_study_crf_fields_html, submit_lock_study_crf_submission,
        submit_mark_study_crf_submission_submitted, submit_publish_study_crf_template,
        submit_respond_study_data_query, submit_schedule_patient_visit,
        submit_set_site_startup_checklist_item, submit_set_study_close_checklist_item,
        submit_set_study_startup_checklist_item, submit_study_phase_transition,
        submit_study_phase_transition_v2, submit_update_study_crf_field,
        submit_update_study_crf_submission_sdv,
    },
...
pub(crate) async fn authenticate_from_dev_headers(
    ctx: &AppContext,
...
<ol>
  ...
  <div style="padding-left:1.15rem;">
    ...
    let create_query_action = selected_project_id
        ...
        .unwrap_or_else(|| "#".to_string());
    let query_panel = build_query_monitor_panel(QueryMonitorPanelInput {
        data_queries: &data_queries,
        submissions: &submissions,
        queryable_submissions: &queryable_submissions,
        selected_project_id,
        selected_query_submission_id,
        query_age_filter: &query_age_filter,
        patient_related_queries_filter,
        now,
        admin_email: admin_email.trim(),
        queries_tab_url: &queries_tab_url,
        submissions_tab_url: &submissions_tab_url,
        stale_queries_url: &stale_queries_url,
        aging_queries_url: &aging_queries_url,
        patient_queries_url: &patient_queries_url,
    });
    let selected_query_submission_hex = query_panel.selected_query_submission_hex;
    let create_query_disabled = query_panel.create_query_disabled;
    let no_query_submission_hint = query_panel.no_query_submission_hint;
    let query_header = query_panel.query_header;
    let query_filter_controls_html = query_panel.query_filter_controls_html;
    let data_queries_html = query_panel.data_queries_html;
    let query_submission_options_html = query_panel.query_submission_options_html;
...
<datalist id="study-submission-options">{query_submission_options_html}</datalist>
"#,
        ...
        query_submission_options_html = query_submission_options_html,
...
pub(crate) fn parse_optional_date(raw: &str) -> Result<Option<chrono::NaiveDate>, ApiError> {
...
pub(crate) fn parse_uuid_field(raw: &str, field_name: &str) -> Result<Uuid, ApiError> {
...
pub(crate) fn optional_non_empty(input: &str) -> Option<&str> {
...
/// Thresholds: stale >30d, aging 7-30d, fresh <=7d.
pub(crate) fn patient_report_age(
...
pub(crate) fn render_cingulum_page(title: &str, body_content: String) -> String {
    format!(
        r#"<!doctype html>
<html lang="en">
<head>
